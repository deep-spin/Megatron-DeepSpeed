...
class BlendableDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        *,
        ...
        assert bool(dataset_features) == bool(
            feature_weights
        ), "must pass both dataset features and weights, or neither"
        self.size = size
        ...
        # also normalize feature weights
        if dataset_features:
            feature_weights = np.array(feature_weights, dtype=np.float64)
            sum_feature_weights = np.sum(feature_weights)
            assert sum_feature_weights > 0.0
            feature_weights /= sum_feature_weights
        ...
        if data_cache_path:
            ...
            print_rank_0(
                f"> loading blendable dataset sample index: {sample_index_path}"
            )
